...
//
// If this is not CLI, this assumes that downstream users of wazero imports wazero as "github.com/mgilbir/andsifr".
// To be precise, the returned string matches the require statement there.
// For example, if the go.mod has "require github.com/mgilbir/andsifr 0.1.2-12314124-abcd",
...
func GetWazeroVersion() (ret string) {
	...
	if ok {
		for _, dep := range info.Deps {
			// Note: here's the assumption that wazero is imported as github.com/mgilbir/andsifr.
			if strings.Contains(dep.Path, "github.com/mgilbir/andsifr") {
				ret = dep.Version
			}
		}
